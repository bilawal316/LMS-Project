    ...
    getProjectMembers: async (namesArray) => {
        try {
            ...
            for (const item of namesArray.response) {
                ...
                console.log("projectid",item.projectId,item.teamsMemberId)
...
console.log("project",project)
                const teamMember = await models.TeamMembers.findOne({
                    where: {
                        teamsMemberId: item.teamMemberId,
                    },
                    attributes: ["userId"],
                });
                console.log("teamMemberId",teamMember)
    ...
    getTaskByUserId: async (query) => {
        try {
            console.log("memberId",query)

            const memberId = await models.TeamMembers.findOne({
                where: {
                    userId: query.userId,
                },
                attributes: ["teamsMemberId"],
            ...
            if (memberId) {
                tasks = await models.Tasks.findAll({
                    where: {
                        teamMemberId: memberId.dataValues.teamsMemberId,
                    }
                })
            }
            return {
